  ...
  function micON() { // When the microphone button is clicked
    if (recording === false) {
      if (records === 0) { // The first time the mic is clicked - inform user
        ...
        //Api.setResponsePayload({output: {text: ['Acepta utilizar el micrófono en tu navegador. Watson escuchará pronto.'], ref: 'STT'}}); // Dialog box output to let the user know we're recording
        records++;
      } else {
        ...
        stream.promise()                                // Once all data has been processed...
          .then(function(data) {                        // ...put all of it into a single array
            ...
            if (data.length !== 0) {                    // If data is not empty (the user said something)
              ...
              Api.setResponsePayload({output: {text: ['Entrada por micrófono cancelada. Por favor vuelve a presionar el botón para hablar con Watson nuevamente.']}}); // If the user clicked the microphone button again to cancel current input
            }
          })
          .catch(function(err) { // Catch any errors made during the promise
            ...
            mic.setAttribute('class', 'inactive-mic'); // Reset our microphone button to visually indicate we aren't listening to user anymore
           // Api.setResponsePayload({output: {text: ['Watson se detuvo después de unos segundos de inactividad. Por favor vuelve a presionar el botón para hablar con Watson nuevamente.']}});
          });
      })
